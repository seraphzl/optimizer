        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if rho_t >= 5:
                    ...
                    if amsgrad:
                        p_data_fp32.addcdiv_(m_t, v_t_max.sqrt().add_(group['eps']), value=-step_size * group['lr'])
                    else:
                        p_data_fp32.addcdiv_(m_t, v_t.sqrt().add_(group['eps']), value=-step_size * group['lr'])
                    p.data.copy_(p_data_fp32)
                elif step_size > 0:
                    # if group['weight_decay'] != 0:
                    #     p_data_fp32.add_(p_data_fp32, alpha=-group['weight_decay'] * group['lr'])
                    p_data_fp32.add_(m_t, alpha=-step_size * group['lr'])
                    p.data.copy_(p_data_fp32)
